...
async def test_combine_centroids(dut):
    ...
    await check_output(
        dut,
        expected_x=100,
        expected_y=50,  # Average of 50 and 60
        expected_z=200
    )
    
    await send_input_data(
        dut,
        local_x=511,  # Max 9-bit value
        local_y=255,  # Max 8-bit value
        uart_z=511,
        uart_y=255
    )
    await check_output(
    ...
    # Alternating data arrival
    dut.local_valid_in.value = 0
    dut.uart_valid_in.value = 0
    await RisingEdge(dut.clk_in)
    
    # Send local data first
    ...
    # Update local data before UART arrives
    ...
    await check_output(
        dut,
        expected_x=160,
        expected_y=90,  # Average of 80 and 100
        expected_z=250
    )
